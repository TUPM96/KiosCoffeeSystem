...
const AdminLogin = () => {
    ...
    const onFinish = async (values) => {
        ...
        try {
            const res = await apiExecutions.adminLogin({
                username: values.username, passwordHash: values.password,
            });
            if (res?.status === 200 || res.status === 201) {
                localStorage.setItem("kiosAdmin", JSON.stringify(res.data));
    ...
    return (<div style={{
        ...
        <Card
            style={{
                width: 400, borderRadius: 16, boxShadow: "0 2px 16px 0 rgba(50,10,107,0.10)",
            }}
        >
            <img src="/app.jpeg" alt="Logo" style={{
                width: 150,
                marginBottom: 20,
                display: 'block',
                marginLeft: 'auto',
